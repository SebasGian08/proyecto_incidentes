...
$(function() {
    ...
    let tabla = $('#tableIncidentes').DataTable({
        ajax: {
            url: "{{ route('incidentes.list_filtros') }}",
            data: function(d) {
    ...
    $(document).on('click', '.gestionar', function() {
        ...
        $.get("{{ route('incidentes.get', ':id') }}".replace(':id', currentId), function(res) {
        ...
        $.get("{{ route('incidentes.historial', ':id') }}".replace(':id', currentId), function(data) {
